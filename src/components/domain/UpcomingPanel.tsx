...
export function UpcomingPanel({ open, onOpenChange }: UpcomingPanelProps) {
    const { dayData, holidays, showBankHolidays } = usePlannerStore();
                        ...
                        {upcomingEvents.length === 0 ? (
                            ...
                            upcomingEvents.map((item) => (
                                <div key={item.date.toISOString()} className="space-y-2">
                                    <h4 className="font-medium text-sm text-muted-foreground sticky top-0 bg-background py-1 z-10 border-b">
                                    ...
                                    <div className="space-y-2">
                                        {item.events.map((event, idx) => (
                                            <div key={idx} className="flex items-center gap-3 p-3 rounded-lg border bg-card text-card-foreground shadow-sm">
                                                <div className={`h-2 w-2 rounded-full shrink-0 ${event.type === 'holiday' ? 'bg-red-500' :
                                                        event.type === 'vacation' ? 'bg-teal-500' : 'bg-blue-500'
                                                    }`} />
